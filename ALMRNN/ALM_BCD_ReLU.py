...
def initialize_ALM_params(Nh,
                          ...
                          submaxiter=100):
    """
    Initialize parameters for the ALM algorithm.
    ...
    - lambda1: Regularization parameter (default: 1 / (Nh * Ny))
    - lambda2: Regularization parameter (default: 1 / (Nh * Nh))
    - lambda3: Regularization parameter (default: 1 / (Nx * Nh))
    - lambda4: Regularization parameter (default: 1 / Nh)
    - lambda5: Regularization parameter (default: 1 / Ny)
    - lambda6: Regularization parameter (default: 1e-8)
...
def initialize_variables(Ny,
                         ...
                         lambda6=None):
    """
    Initialize variables for the ALM algorithm.
    ...
    - xi1, xi2, gamma1, gamma2: ALM-related parameters
...
def alm_bcd_ReLU_optimization(dataset_name,
                              Ny=1,
                              Nh=20,
                              maxiter=10,
                              submaxiter=100,
    ...
    if print_option >= 1:
        print(
            "\nWe use an Augmented Lagrangian Method to solve the following RNN training problem with ReLU:"
        )
    ...
    for k in range(maxiter):
        ...
        for j in range(params["submaxiter"]):
            ...
            # Update \bf{w}
            zin_kj, W_kj, V_kj, b_kj = BCD_closed.updateZin_closed(
                W_kjm1, V_kjm1, b_kjm1, u_kjm1, ht_kjm1, x_trainset,
                params["xi1"], params["gamma1"], lambda2, lambda3, lambda4, Nx,
                Nh, T)

            # Update \bf{a}
            zout_kj = BCD_closed.updateZout_closed(ht_kjm1, y_trainset,
                                                   lambda1, lambda5, Nh, Ny, T)
            A_kj = zout_kj[:Ny * Nh].reshape((Ny, Nh), order="F")
            c_kj = zout_kj[Ny * Nh:Ny * Nh + Ny].reshape((Ny, 1))

            # Update \bf{h}
            ht_kj = BCD_closed.updateH_closed(A_kj, W_kj, V_kj, b_kj, c_kj,
                                              ut_kjm1, x_trainset, y_trainset,
                                              params["gamma1"],
                                              params["gamma2"], params["xi1"],
                                              params["xi2"], Nx, Nh, Ny, T)
            h_kj = ht_kj.T.flatten()
            h_kj = h_kj.reshape(T * Nh, 1)

            # Update \bf{u}
            u_kj = BCD_closed.updateU_closed(
                zin_kj, zout_kj, A_kj, W_kj, V_kj, b_kj, h_kj, ht_kj, u_kjm1,
                params["xi1"], params["xi2"], params["gamma1"],
                params["gamma2"], lambda1, lambda2, lambda3, lambda4, lambda6,
                sigma, x_trainset, y_trainset, Nh, Nx, Ny, T)
        ...
        # Feasibility
        FeasVi_h.append(np.linalg.norm(h_submin - toolfunc.ReLU(u_submin)))
        FeasVi_u.append(
            np.linalg.norm(
                u_submin -
        ...
        params["xi2"] += params["gamma2"] * (h_submin -
                                             toolfunc.ReLU(u_submin))
