...
def GradALM_h(W, V, b, A, c, z_in, z_out, u, ut, h, ht, x_dataset, y_dataset,
    ...
    grad_g8 = xi2
    grad_g9 = gamma2 * (h - toolfunc.ELU(u))
...
def GradALM_u(W, V, b, u, ut, h, ht, z_in, x_dataset, xi1, xi2, gamma1, gamma2,
    ...
    u = u.reshape(Nh * T, 1)
    h = h.reshape(Nh * T, 1)
    grad_g7 = gamma1 * (u.reshape(Nh * T, 1) -
                        ALfunc.PsiX(ht, x_dataset, z_in, Nx, Nh, T)) + xi1
    grad_gu = -gamma1 * (
        toolfunc.ELU_grad(u).reshape(Nh * T, 1) *
        (h - toolfunc.ELU(u).reshape(Nh * T, 1) + xi2 / gamma1))
    return grad_g7 + grad_gu


def GradALM_u_block(W, V, b, u, ut, u_last, h, ht, z_in, x_dataset, xi1, xi2,
                    gamma1, gamma2, T, Nx, Nh, lambda6, sigma):
    # This function is to calculate the gradient for lagrange function L_gamma1(z, h, u) of u
    # Calculate the gradient of g7
    u = u.reshape(Nh * T, 1)
    h = h.reshape(Nh * T, 1)
    grad_g7 = gamma1 * (u.reshape(Nh * T, 1) -
                        ALfunc.PsiX(ht, x_dataset, z_in, Nx, Nh, T)) + xi1
    grad_gu = -gamma1 * (toolfunc.ELU_grad(u).reshape(Nh * T, 1) *
                         (h - toolfunc.ELU(u).reshape(Nh * T, 1) + xi2 / gamma1
                          )) + sigma * (u.reshape(Nh * T, 1) - u_last.reshape(
                              Nh * T, 1)) + 2 * lambda6 * u.reshape(Nh * T, 1)
    return grad_g7 + grad_gu
